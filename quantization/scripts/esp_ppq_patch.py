...
def apply_esp_ppq_patches():
    ...
    def patched_Slice_forward(
        ...
        **kwargs
        ...
        ASSERT_NUM_OF_INPUT(
            op=op, values=values, min_num_of_input=3, max_num_of_input=5
        )
    ...
    def patched_Gather_forward(
        ...
        **kwargs
        ...
        if op.type == "Gather":
            array_idx = [
                indices if axis == i else slice(dim)
                for i, dim in enumerate(input_data.shape)
            ]
            # Fix: Tuple conversion ensures compatibility with newer PyTorch
