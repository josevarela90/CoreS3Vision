...
def register_mod_op():
    ...
    try:
        ...
        print(
            "Could not import register_operation_handler from esp_ppq.executor. Skipping Mod patch."
        )
...
def patch_detect_forward(model):
    ...
    def new_forward(self, x):
        ...
        for i in range(self.nl):
            one2one.append(
                torch.cat((self.one2one_cv2[i](x[i]), self.one2one_cv3[i](x[i])), 1)
            )
...
def get_exclusive_ancestors(graph, target_outputs, excluded_outputs):
    ...
    target_vars = [
        graph.outputs[name] for name in target_outputs if name in graph.outputs
    ]
    excluded_vars = [
        graph.outputs[name] for name in excluded_outputs if name in graph.outputs
    ]
