...
class LetterboxSquare:
    """Resize preserving aspect ratio and pad with RGB 114."""
...
class CaliDataset(Dataset):
    ...
    def __init__(self, path):
        ...
        # Match ESP-DL/CoreS3 preprocessing:
        # letterbox -> RGB -> [0,1]
        self.transform = transforms.Compose(
            [
                LetterboxSquare(QATConfig.IMG_SZ),
                transforms.ToTensor(),
...
class SubsetCaliDataset(CaliDataset):
    ...
    def __init__(self, path, max_images=QATConfig.CALIB_MAX_IMAGES):
        ...
        self.imgs_path = [
            p for p in self.imgs_path if p.lower().endswith(valid_extensions)
        ]
...
def get_calibration_loader(data_cfg):
    ...
    if not calibration_path:
        ...
        print(
            f"Could not deduce exact image folder from {QATConfig.DATA_YAML_FILE}, defaulting to {calibration_path}"
        )
    else:
