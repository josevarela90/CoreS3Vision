...
def ppq_graph_inference(executor, task, inputs, device, model_meta):
    ...
    if task == "detect":
        ...
        preds = {"feats": raw_x, "boxes": cat_boxes, "scores": cat_scores}
...
class QuantizedModelValidator(BaseValidator):
    ...
    def __call__(self, trainer=None, model=None, executor=None):
        ...
        for batch_i, batch in enumerate(bar):
            ...
            # Inference
            with dt[1]:
                preds = ppq_graph_inference(
                    executor, "detect", batch["img"], self.device, model_meta
                )
            # Loss (skipped)
            ...
            if self.args.plots and batch_i < getattr(
                QATConfig, "VAL_PLOT_MAX_BATCHES", 3
            ):
